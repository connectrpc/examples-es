...
function create_updated_store() {
    ...
    const interval = +(
        /** @type {string} */ (
            import.meta.env.VITE_SVELTEKIT_APP_VERSION_POLL_INTERVAL
        )
    )
...
function parse(components, dictionary, matchers) {
    const routes = Object.entries(dictionary).map(
        ([id, [a, b, has_shadow]]) => {
...
function create_client({ target, session, base, trailing_slash }) {
    ...
    async function update(url, redirect_chain, no_cache, opts, callback) {
        ...
        if (navigation_result.redirect) {
            if (
                ...
                redirect_chain.includes(url.pathname)
            ) {
                navigation_result = await load_root_error_page({
                    status: 500,
            ...
            try {
                ...
                if (changed_since_last_render) {
                    ...
                    if (is_shadow_page) {
                        ...
                        if (res.ok) {
                            const redirect = res.headers.get(
                                'x-sveltekit-location'
                            )
    ...
    async function navigate({
        ...
        const normalized = new URL(
            url.origin + pathname + url.search + url.hash
        )
        ...
        _start_router: () => {
            ...
            addEventListener('visibilitychange', () => {
                if (document.visibilityState === 'hidden') {
                    ...
                    try {
                        sessionStorage[SCROLL_KEY] =
                            JSON.stringify(scroll_positions)
                    } catch {
                        // do nothing
                    }
                }
            ...
            addEventListener('click', (event) => {
                ...
                navigate({
                    url,
                    scroll: a.hasAttribute('sveltekit:noscroll')
                        ? scroll_state()
                        : null,
                    keepfocus: false,
                    redirect_chain: [],
