    ...
    if (resolve_opts.ssr) {
        branch.forEach(
            ({ node, props: props2, loaded, fetched, uses_credentials }) => {
                ...
                if (node.styles)
                    Object.entries(node.styles).forEach(([k, v]) =>
                        styles.set(k, v)
                    )
        ...
        const props = {
            stores: {
                ...
                session: {
                    ...
                    subscribe: (fn) => {
                        is_private =
                            (cache == null ? void 0 : cache.private) ?? true
            ...
            fetch: async (resource, opts = {}) => {
                ...
                if (is_asset || is_asset_html) {
                    ...
                    response = await respond(
                        new Request(new URL(requested, event.url).href, {
                            ...opts,
                        }),
                        options,
                        {
                ...
                if (error2) {
                    while (i--) {
                        if (route.b[i]) {
                            ...
                            const error_node = await options.manifest._.nodes[
                                index
                            ]()
                            let node_loaded
                            let j = i
...
async function respond(request, options, state) {
    ...
    const method_override =
        (_a = url.searchParams.get(parameter)) == null
            ? void 0
            : _a.toUpperCase()
    if (method_override) {
    ...
    const removed = (property, replacement, suffix = '') => ({
        get: () => {
            throw new Error(
                `event.${property} has been replaced by event.${replacement}` +
                    suffix
            )
        },
    })
    ...
    try {
        const response = await options.hooks.handle({
            ...
            resolve: async (event2, opts) => {
                ...
                if (
                    (_a2 = state.prerendering) == null ? void 0 : _a2.fallback
                ) {
                    return await render_response({
                        event: event2,
                        options,
