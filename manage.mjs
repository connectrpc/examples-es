...
function main() {
    ...
    switch (command) {
        ...
        case "ci":
          ...
          break;
        case "update-dependabot-yml":
          const cwd = process.cwd();
          const directories = packages.map(pkg => pkg.path.substring(cwd.length + 1));
          updateDependabotYaml(".github/dependabot.yml", directories);
          break;
        default:
 ...
 **/
function updateDependabotYaml(filePath, directories) {
  const entries = directories.map(dir =>`
...
`).join("");
  ...
  const cStart = "# start generated by ./manage.mjs update-dependabot-yml";
  const cEnd = "# end generated by ./manage.mjs update-dependabot-yml";
  ...
  const newContent = head + entries + foot;
